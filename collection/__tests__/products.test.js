...
describe('Products API', () => {
  ...
  it('respond properly to a update request to /api/v1/products/:id', () => {
    let obj = {name: 'orange', price: 15, quantity_in_stock: 250 };
    return mockRequest.post('/api/v1/products')
      .send(obj)
      .then(data=>{
        return mockRequest.put(`/api/v1/products/${data.body._id}`)
          .send({ name: 'ITEM  UPDATED', price: 22, quantity_in_stock: 123})
          .then(results=>{
            expect(results.status).toBe(200);
            expect(results.body.name).toEqual('ITEM  UPDATED');
            expect(results.body.price).toEqual(22);
            expect(results.body.quantity_in_stock).toEqual(123);
          });
      });
  });
});